...
# https://search.nixos.org/options and in the NixOS manual (`nixos-help`).
{
  ...
  dufs-merge-auth =
    ...
    '';
  # ROCmFPX fork of llama.cpp: adds ROCmFP4 weight formats + MTP speculative
  # decoding. Vulkan build, used only by the dedicated llama-cpp-rocmfp4 service
  # to serve qwen3.6-35B at Q4_0_ROCMFP4_STRIX_LEAN. Upstream llama.cpp can't
  # decode ROCmFP4 tensors.
  ...
  # ROCmFP4 STRIX_LEAN quantization of Qwen3.6-35B-A3B (BF16 + imatrix derived,
  # gsrunion). Served by the dedicated llama-cpp-rocmfp4 service. ~18 GiB.
  qwen36-35b-rocmfp4-strixlean = pkgs.fetchurl {
...
in {
  ...
  # MTP self-spec). Uses the ROCmFPX fork which is the only build that decodes
  # ROCmFP4 tensors. Runs on its own port so the upstream llama-cpp service
  # (port 11404) stays on the standard Vulkan build for the other models.
  systemd.services.llama-cpp-rocmfp4 = lib.mkIf (my-system.enableLlamaCpp or false) {
    ...
    serviceConfig = {
      ...
      ExecStart =
        ...
        + "--host ${my-system.tailscaleIp or "127.0.0.1"} "
        + "--port 11405 "
        + "--model ${qwen36-35b-rocmfp4-strixlean} "
        + "--ctx-size 262144 "
